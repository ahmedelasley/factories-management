    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content modal-content-demo">
            <div class="modal-header">
                <h6 class="modal-title">{{ __('Update', ['type' => __('Material')]) }}</h6>
            ...
            <div class="modal-body">
                <form id="form">
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400 text-center">
                        {{ __('This action will change the status of', ['type' => __('Material')]) }} <strong>{{ $model?->name }}</strong> {{ __('to') }} <strong>{{ $model?->status == "Active" ? __('Active') : __('Inactive') }}</strong>.
                    </p>
                </form>
            </div>
